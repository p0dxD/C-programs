...
int create_msg(char *buf, char *msg, char *name) {
	...
	if (tokenc < 3 || !strcmp(tokens[1], name)) {
		return 1;
	}
	strcpy(msg, "MSG ");
...
int writeMsgToFile(char *buf, char *msg, char *name, char *filename, pthread_mutex_t count_mutex) {
	...
	sfwrite(&count_mutex, file, "%s, %s, %s, %s, %s, %s", getDateTimeStamp(), name, "MSG", "to", tokens[1], strtok(msg, "\n"));
	Fclose(file);
	return 0;
}
...
/*The ones missing from the book*/
void Flock(int fd, int operation) {
	int rc;
	if ((rc = flock(fd, operation)) < 0)
		fprintf(stderr, "%s: %s\n", "flock failed.", strerror(errno));
}
